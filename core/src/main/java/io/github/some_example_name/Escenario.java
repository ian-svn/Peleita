...
Escenario extends ApplicationAdapter {
    ...
    @Override
    public void create() {
        musicaFondo = Gdx.audio.newMusic(Gdx.files.internal("cancionFondo.mp3"));
        ...
        if(!menu.isActivo() && !menu.isSeguro()) {
            ...
            if (Gdx.input.isKeyPressed(Input.Keys.TAB)) {
                ...
                for(ObjetoPasivo obj : getLuchador().getObjetos()){
                    aux++;
                    batch.draw(obj.getSprite(), 50f, ALTO - ALTO / 6 - 60f * aux, obj.getANCHO(), obj.getALTO());
                }
